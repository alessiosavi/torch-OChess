...
class GUIInterface:
    ...
    def __init__(self, model: nn.Module, device: str = "cuda", square_size: int = 80):
        """
    ...
    def _create_widgets(self):
        ...
        self.move_label = tk.Label(
            info_frame, text="Move: 1", font=("Helvetica", 11), fg="white", bg="#333333"
        )
        ...
        self.last_move_label = tk.Label(
            info_frame, text="Last: -", font=("Helvetica", 11), fg="white", bg="#333333"
        )
        ...
        self.new_game_btn = tk.Button(
            btn_frame, text="New Game", command=self._new_game_dialog, width=15
        )
        self.new_game_btn.pack(pady=3)

        self.undo_btn = tk.Button(
            btn_frame, text="Undo Move", command=self._undo_move, width=15
        )
        self.undo_btn.pack(pady=3)

        self.flip_btn = tk.Button(
            btn_frame, text="Flip Board", command=self._flip_board, width=15
        )
        self.flip_btn.pack(pady=3)

        self.quit_btn = tk.Button(
            btn_frame, text="Quit", command=self.root.quit, width=15
        )
    ...
    def _draw_board(self):
        ...
        for rank in range(8):
            for file in range(8):
                ...
                if self.selected_square == square:
                    ...
                    square == self.last_move.from_square
        ...
        for square in chess.SQUARES:
            ...
            if piece:
                ...
                if flip:
                    x = (7 - file) * self.square_size + self.square_size // 2
        ...
        for rank in range(8):
            ...
            if flip:
                ...
                y = (7 - rank) * self.square_size + self.square_size // 2
            self.canvas.create_text(
                8, y, text=label, font=("Helvetica", 10), fill="#666666"
            )
            ...
            for m in self.legal_moves_from_selected:
                if m.to_square == clicked_square:
                    ...
                    if self.board.piece_at(
                        ...
                        move = chess.Move(
                            self.selected_square, clicked_square, promotion=chess.QUEEN
                        )
                    else:
                        move = m
                    break
